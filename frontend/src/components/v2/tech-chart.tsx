...
export default function TechnicalCharts() {
  ...
  const fetchSMAData = async (ticker: string, timeRange: string) => {
    ...
    try {
      const response = await fetch(
        `${getApiUrl()}/api/v1/getIntervalSMAData?ticker=${ticker}&scale=${timeRange}`,
        {
          method: "GET",
          headers: {
  ...
  const fetchEMAData = async (ticker: string, timeRange: string) => {
    ...
    try {
      const response = await fetch(
        `${getApiUrl()}/api/v1/getIntervalEMAData?ticker=${ticker}&scale=${timeRange}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchRSIData = async (ticker: string, timeRange: string) => {
    ...
    try {
      const response = await fetch(
        `${getApiUrl()}/api/v1/getIntervalRSIData?ticker=${ticker}&scale=${timeRange}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const latestPrice =
    smaData.length > 0 ? smaData[smaData.length - 1].close : null;
        ...
        <CardHeader className="flex items-center gap-5 space-y-0 border-b py-5 sm:flex-row">
          <div className="grid flex-1 gap-1 text-center sm:text-left">
            <CardTitle className="text-2xl">
              {selectedTicker} {latestPrice ? `$${latestPrice.toFixed(2)}` : ""}
            </CardTitle>
