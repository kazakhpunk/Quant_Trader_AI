...
const CATEGORIES: AssetCategory[] = ["overall", "region", "rating", "grade", "sector", "etf"];

function featureVector(c: AssetDto): number[] {
  // Category one-hot + source one-hot. PCA on these encodings will cluster
  // assets by category — the closest meaningful "map" we can draw without
  // having uniform metadata across mixed FRED + Yahoo entries.
  return [
    ...CATEGORIES.map((k) => (c.category === k ? 1 : 0)),
    c.source === "fred" ? 1 : 0,
    c.source === "yahoo" ? 1 : 0,
    c.region === "LatAm" ? 1 : 0,
    c.region === "EMEA" ? 1 : 0,
